...
#pragma omp parallel for num_threads(MAX_THREADS)
    for (int idx = 0; idx < imuSelected.size(); idx++)
    {
        ...
        vector<SO3d> xr_local;
        vector<Vector3d> xp_local;
    ...
    double *cost)
{
    ...
    if (knot_x_keep.size() != 0)
    {
        ...
        // Update the hessian
        rprior_sparse = rprior.sparseView();
        rprior_sparse.makeCompressed();
        Jprior_sparse = Jprior.sparseView();
        Jprior_sparse.makeCompressed();

        Hprior_sparse = Hprior.sparseView();
        Hprior_sparse.makeCompressed();
        bprior_sparse = bprior.sparseView();
        bprior_sparse.makeCompressed();
    ...
    slict::TimeLog &tlog)
{
    ...
    SparseMatrix<double> Jsparse = JACOBIAN.sparseView();
    Jsparse.makeCompressed();
    SparseMatrix<double> Jtp = Jsparse.transpose();
    SparseMatrix<double> H = Jtp * Jsparse;
    MatrixXd b = -Jtp * RESIDUAL;

    if (fuse_marg && Hprior_sparse.rows() > 0 && bprior_sparse.rows() > 0)
    {
        H += Hprior_sparse;
        b += bprior_sparse;
    ...
    SparseMatrix<double> I(H.cols(), H.cols());
    I.setIdentity();
    ...
    if (iter == 0 && fuse_marg)
    {
        ...
        SparseMatrix<double> Jsparse = Jmarg.sparseView();
        Jsparse.makeCompressed();
        SparseMatrix<double> Jtp = Jsparse.transpose();
        SparseMatrix<double> H = Jtp * Jsparse;
        ...
        {
            if (x_knot_marg.find(knot_x.second) != x_knot_marg.end() || x_knot_keep.find(knot_x.second) != x_knot_keep.end())
        ...
        if (Hprior_final_reduced.rows() != 0 && bprior_final_reduced.rows() != 0 && all_retained == true)
        {
            ...
            SparseMatrix<double> H_extr_sparse = Hprior_final_reduced.sparseView();
            H_extr_sparse.makeCompressed();
            SparseMatrix<double> b_extr_sparse = bprior_final_reduced.sparseView();
            b_extr_sparse.makeCompressed();
